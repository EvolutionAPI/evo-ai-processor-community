...
Fetches PayPal integration configuration from the database and prepares
...
async def get_paypal_mcp_config(
    ...
    """
    Get PayPal MCP server configuration.
    ...
    """
    default_url = "https://mcp.paypal.com/mcp"
    ...
    if db:
        try:
            ...
            for integration in integrations:
                if integration.get("provider", "").lower() == "paypal":
                    paypal_integration = integration
                    break

            if paypal_integration:
                integration_config = paypal_integration.get("config", {})
                ...
                logger.info(
                    f"PayPal integration found for agent {agent_id}: "
                ...
                if access_token:
                    ...
                    logger.info(f"PayPal MCP config loaded with access token for agent {agent_id}")
                else:
                    logger.warning(f"PayPal integration found but no access token for agent {agent_id}")

                # Store selected tools in config for filtering (always set, even if empty)
                config["tools"] = tools if tools else []
                if tools:
                    logger.info(f"PayPal MCP config includes {len(tools)} selected tools: {tools[:5]}...")
                else:
                    logger.info(f"PayPal MCP config: no tools selected, will use all available tools")
            else:
                logger.debug(f"No PayPal integration found for agent {agent_id}")
        except Exception as e:
            logger.error(f"Error loading PayPal integration config: {e}")
