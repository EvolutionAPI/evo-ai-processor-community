...
router = APIRouter(
    prefix="/accounts/{account_id}/agents/{agent_id}/integrations/supabase",
    tags=["supabase"],
)
...
async def get_configuration(
    ...
    try:
        ...
        config = await get_integration_config(db, account_id, agent_id, "supabase", load_from_service=service._load_credentials)
        ...
        if not config:
            ...
            try:
                ...
                success = await upsert_agent_integration(
                    db, agent_id, account_id, "supabase", default_config
                )
                if success:
...
async def discover_tools(
    ...
    tools = await discover_tools_endpoint(
        account_id, agent_id, "supabase", "https://mcp.supabase.com/mcp", db
    )
...
async def save_configuration(
    ...
    try:
        ...
        stored_config = await get_integration_config(db, account_id, agent_id, "supabase", load_from_service=service._load_credentials)
        ...
        success = await upsert_agent_integration(
            db, agent_id, account_id, "supabase", updated_config
        )
